...
require_once 'api_ai_analyze.php';  // Import hàm chuẩn hóa loại giày
...
/**
 * Tính ngày Tết Nguyên Đán theo lịch âm (thuật toán chuyển đổi)
 */
function getVietnamLunarNewYear($year) {
    // Sử dụng API hoặc thuật toán tính toán
    // Đây là dữ liệu tham khảo cho các năm gần
    $lunarNewYearDates = [
    ...
    // Nếu không có, thử lấy từ API hoặc tính toán
    // Sử dụng thuật toán chuyển đổi Dương lịch -> Âm lịch
    // Tết thường rơi vào khoảng 21/1 - 20/2
    return null;
}
...
function getRestockSuggestions($conn, $warehouse_id, $month, $region) {
    try {
        ...
        foreach ($hot_shoes as $shoe_type) {
            // Sử dụng hàm chuẩn hóa từ api_ai_analyze.php
        ...
        // 5. BACKUP PRODUCTS - TẮT để chỉ hiển thị sản phẩm đúng gợi ý
        // Nếu bật lại, uncomment phần if (count($suggestions) < 6) bên dưới
        if (false && count($suggestions) < 6) {
            // Tạo danh sách các loại giày nên ưu tiên (từ hot_shoes)
            $hot_types_for_backup = [];
            foreach ($hot_shoes as $hot_shoe) {
                $mapped = $shoe_mapping[$hot_shoe] ?? standardizeProductType($hot_shoe);
                if (!in_array($mapped, $hot_types_for_backup)) {
                    $hot_types_for_backup[] = $mapped;
                }
            }
            
            error_log("🔍 Backup search for hot types: " . implode(', ', $hot_types_for_backup));
            
            // Tạo WHERE clause linh hoạt để tìm các loại giày HOT
            $type_conditions = [];
            foreach ($hot_types_for_backup as $hot_type) {
                $safe_type = str_replace("'", "''", $hot_type); // SQL escape
                $type_conditions[] = "(p.type LIKE '%{$safe_type}%' OR p.name LIKE '%{$safe_type}%')";
            }
            
            // Nếu không có điều kiện type, mở rộng tìm kiếm theo từ khóa
            if (empty($type_conditions)) {
                // Fallback: tìm theo từ khóa chung từ hot_shoes
                $keywords = ['sandal', 'dép', 'giày thoáng', 'chống nước', 'sneaker'];
                foreach ($keywords as $keyword) {
                    $type_conditions[] = "(p.name LIKE '%{$keyword}%' OR p.description LIKE '%{$keyword}%')";
                }
            }
            
            $type_where = !empty($type_conditions) ? '(' . implode(' OR ', $type_conditions) . ')' : '1=1';
            ...
            $sql_backup = "SELECT 
                    ...
                    WHERE p.warehouse_id = :wh_id3
                        ...
                        AND " . $type_where . "
                    GROUP BY p.product_id, p.name, p.type, p.description, p.brand, pi.file_path
                    HAVING current_stock < 200
                    ORDER BY 
                        CASE 
                            WHEN p.type LIKE '%Sandal%' THEN 1
                            WHEN p.name LIKE '%sandal%' THEN 2
                            WHEN p.name LIKE '%dép%' THEN 3
                            ELSE 4
                        END,
                        current_stock ASC, 
                        total_sold DESC
            ...
            $stmt_backup->bindParam(':wh_id1', $warehouse_id);
            $stmt_backup->bindParam(':wh_id2', $warehouse_id);
            $stmt_backup->bindParam(':wh_id3', $warehouse_id);
            $stmt_backup->execute();
            while ($row = $stmt_backup->fetch(PDO::FETCH_ASSOC)) {
                if (isset($processed_products[$row['product_id']]) || count($suggestions) >= 12) {
                    continue;
                }
                ...
                // Chuẩn hóa product type cho backup products
                $backup_type_standardized = standardizeProductType($row['product_type']);
                $backup_name_lower = strtolower($row['product_name']);
                $backup_desc_lower = strtolower($row['description']);
                
                // Kiểm tra xem có thuộc hot_shoes không - CHẶT CHẼ HƠN
                $is_hot_type = false;
                $matched_hot_type = '';
                
                foreach ($hot_types_for_backup as $hot_type) {
                    $hot_type_lower = strtolower($hot_type);
                    
                    // Kiểm tra STRICT: Type phải match HOẶC tên phải có từ khóa chính xác
                    if ($backup_type_standardized === $hot_type) {
                        // Type chính xác
                        $is_hot_type = true;
                        $matched_hot_type = $hot_type;
                        break;
                    } elseif ($hot_type_lower === 'sandal') {
                        // Cho Sandal: PHẢI có "sandal" hoặc "dép" nhưng KHÔNG ĐƯỢC có "cao gót"
                        if ((strpos($backup_name_lower, 'sandal') !== false || 
                             strpos($backup_name_lower, 'dép') !== false ||
                             strpos($backup_desc_lower, 'sandal') !== false) &&
                            strpos($backup_name_lower, 'cao gót') === false &&
                            strpos($backup_name_lower, 'cao g') === false) {
                            $is_hot_type = true;
                            $matched_hot_type = $hot_type;
                            break;
                        }
                    } elseif (strpos($backup_type_lower, $hot_type_lower) !== false || 
                              strpos($backup_name_lower, $hot_type_lower) !== false) {
                        $is_hot_type = true;
                        $matched_hot_type = $hot_type;
                        break;
                    }
                ...
                // Nếu không thuộc loại HOT, skip (chỉ lấy backup từ loại giày HOT)
                if (!$is_hot_type) {
                    error_log("⏩ SKIP backup (not hot type): {$row['product_name']} ({$backup_type_standardized})");
                    continue;
                }
                
                error_log("✅ BACKUP product added: {$row['product_name']} ({$backup_type_standardized}) - Matched: $matched_hot_type - Stock: $stock_level");
                
                $priority = $stock_level < 30 ? 'HIGH' : ($stock_level < 100 ? 'MEDIUM' : 'LOW');
                $avg_daily_sales = $sales_velocity > 0 ? $sales_velocity / 30 : 0.5;
                $days_of_stock = $stock_level > 0 ? $stock_level / max($avg_daily_sales, 0.1) : 0;
                $base_quantity_backup = max(50, ceil($avg_daily_sales * 60));
                
                // ÁP DỤNG HỆ SỐ LỄ HỘI cho backup products
                $suggested_quantity = ceil($base_quantity_backup * $event_boost);
                
                // Kiểm tra nếu thuộc loại ưu tiên sự kiện
                $event_priority_boost_backup = false;
                if (!empty($event_preferences)) {
                    foreach ($event_preferences as $preferred_type) {
                        if (strpos($backup_type_standardized, $preferred_type) !== false) {
                            $event_priority_boost_backup = true;
                            if ($priority === 'MEDIUM') $priority = 'HIGH';
                            break;
                        }
                    }
                ...
                // Tạo lý do phù hợp với khí hậu
                $backup_reason = "Phù hợp khí hậu tháng $month ({$month_data['season']})";
                if ($stock_level < 50) {
                    $backup_reason .= " + Tồn kho thấp";
                }
                if ($event_note) {
                    $backup_reason .= " | 🎉 " . $event_note;
                }
                ...
                $suggestions[] = [
                    ...
                    'product_name' => $row['product_name'],
                    'image_url' => $row['image_url'] ?: 'default-shoe.jpg',
                    'category' => $backup_type_standardized ?: 'Sneaker',  // Sử dụng type đã chuẩn hóa
                    'current_stock' => $stock_level,
                    'total_sold_30days' => $sales_velocity,
                    'priority' => $priority,
                    'suggested_quantity' => (int)$suggested_quantity,
                    ...
                    'demand_type' => $matched_hot_type ?: implode(', ', array_slice($hot_shoes, 0, 2)), // Hiển thị loại matched
                    'price' => (float)$row['price'],
                    'brand' => $row['brand'] ?: '',
                    'region' => $region,
                    'event_boost' => $event_boost,
                    'is_event_priority' => $event_priority_boost_backup
                ];
            }
        }
        
        // 6. Sắp xếp theo mức độ ưu tiên
...
/**
 * Đánh giá vòng quay kho theo chuẩn VN
 */
function getTurnoverRating($turnover) {
    if ($turnover >= 3) return ['rating' => 'Excellent', 'note' => 'Vòng quay nhanh, hiệu quả cao'];
    if ($turnover >= 2) return ['rating' => 'Good', 'note' => 'Vòng quay tốt'];
    if ($turnover >= 1) return ['rating' => 'Average', 'note' => 'Vòng quay bình thường'];
    if ($turnover >= 0.5) return ['rating' => 'Below Average', 'note' => 'Vòng quay chậm, cần cải thiện'];
    return ['rating' => 'Poor', 'note' => 'Hàng ế, cần hành động khẩn cấp'];
}

/**
 * Đánh giá mức tồn kho
 */
function getStockLevelRating($ending_qty, $monthly_sales) {
    if ($monthly_sales <= 0) return ['rating' => 'No Sales', 'note' => 'Không có bán hàng trong tháng'];
    
    $months_of_inventory = $ending_qty / $monthly_sales;
    
    if ($months_of_inventory < 0.5) return ['rating' => 'Critically Low', 'note' => 'Thiếu hàng nghiêm trọng'];
    if ($months_of_inventory < 1) return ['rating' => 'Low', 'note' => 'Mức tồn kho thấp'];
    if ($months_of_inventory <= 2) return ['rating' => 'Optimal', 'note' => 'Mức tồn kho tối ưu'];
    if ($months_of_inventory <= 4) return ['rating' => 'High', 'note' => 'Tồn kho cao'];
    return ['rating' => 'Excessive', 'note' => 'Tồn kho quá mức, nguy cơ ế hàng'];
}
