 ...
 * - Quét QR bằng camera hoặc upload ảnh
 * - Quản lý danh sách QR codes
 * - In lại QR code
 * - Xem lịch sử nhập kho qua QR
 */

class QRCodeManager {
    /**
     * KHỞI TẠO QR CODE MANAGER
     * 
     * @param {Object} config - Cấu hình tùy chọn
     * @param {string} config.apiUrl - URL của API xử lý QR (mặc định: api_quan_ly_ma_qr.php)
     * @param {boolean} config.debug - Bật/tắt chế độ debug (mặc định: true)
     * 
     * Các thuộc tính được khởi tạo:
     * - this.apiUrl: Đường dẫn API endpoint
     * - this.scanner: Đối tượng Html5QrcodeScanner (null ban đầu)
     * - this.isScanning: Trạng thái đang quét hay không
     * - this.debugMode: Chế độ ghi log chi tiết
     */
    constructor(config = {}) {
        // Thiết lập URL API, mặc định là 'api_quan_ly_ma_qr.php'
        this.apiUrl = config.apiUrl || 'api_quan_ly_ma_qr.php';
        
        // Đối tượng scanner (null khi chưa khởi động camera)
        this.scanner = null;
        
        // Cờ đánh dấu trạng thái đang quét
        this.isScanning = false;
        
        // Bật debug mode mặc định để ghi log chi tiết
        this.debugMode = typeof config.debug !== 'undefined' ? config.debug : true;

        // Khởi tạo các event listeners và modals
        this.init();

        // Tải trước thư viện QR để quét nhanh như Zalo
        this.preloadQRLibrary();

        if (this.debugMode) {
            console.log('🚀 QRCodeManager đã khởi tạo với chế độ debug BẬT');
        }
    }
    
    /**
     * TẢI TRƯỚC THỦ VIỆN QR SCANNER
     * 
     * Mục đích: Tải html5-qrcode library ngay sau khi khởi tạo class
     * để khi người dùng bấm nút scan, camera mở ngay lập tức (như Zalo)
     * 
     * Quy trình:
     * 1. Đợi 1 giây sau khi trang load
     * 2. Kiểm tra xem thư viện đã có chưa
     * 3. Nếu chưa có -> tải về từ CDN
     * 4. Khi tải xong -> sẵn sàng quét QR ngay
     * 
     * Lợi ích:
     * - Giảm thời gian chờ khi người dùng bấm scan
     * - Trải nghiệm mượt mà như ứng dụng native
     */
    preloadQRLibrary() {
        // Bắt đầu preloading sau 1 giây (không block UI)
        setTimeout(() => {
            // Chỉ tải nếu thư viện chưa có
            if (typeof Html5QrcodeScanner === 'undefined') {
                console.log('🚀 Đang tải trước thư viện QR scanner để quét nhanh như Zalo...');
                this.loadQRScannerLibrary().then(() => {
                    console.log('✅ Thư viện QR đã tải xong - sẵn sàng quét ngay lập tức!');
                }).catch(error => {
                    console.log('⚠️ Tải trước thất bại, sẽ tải khi cần:', error);
                });
            }
        }, 1000); // Delay 1 giây để không ảnh hưởng trang load
    }
    
    /**
     * KHỞI TẠO CÁC THÀNH PHẦN CỦA MANAGER
     * 
     * Gọi 2 hàm chính:
     * 1. bindEvents() - Gắn các event listeners cho các nút bấm
     * 2. setupModalHandlers() - Tạo các modal popups cần thiết
     */
    init() {
        this.bindEvents();
    ...
    /**
     * GẮN CÁC SỰ KIỆN UI (EVENT LISTENERS)
     * 
     * Sử dụng jQuery event delegation $(document).on() để:
     * - Hoạt động với các element được tạo động
     * - Không bị mất event khi DOM thay đổi
     * 
     * Các sự kiện được gắn:
     * 1. .btn-generate-qr - Tạo QR mới cho sản phẩm
     * 2. .btn-reprint-qr - In lại QR đã có
     * 3. .btn-view-product-qrs - Xem danh sách QR của sản phẩm
     * 4. .btn-scan-qr - Bắt đầu quét QR bằng camera
     * 5. .btn-stop-scan - Dừng quét QR
     * 6. #manual-qr-input - Nhập QR thủ công (không scan)
     */
    bindEvents() {
        // SỰ KIỆN: Nút tạo QR code mới
        // Lấy product_id và variant_id từ data attributes
        ...
        // SỰ KIỆN: Nút in lại QR code (reprint)
        // Lấy qr_code từ data attribute
        $(document).on('click', '.btn-reprint-qr', (e) => {
            const qrCode = $(e.target).data('qr-code');
            this.reprintQR(qrCode);
        });
        
        // SỰ KIỆN: Nút xem tất cả QR codes của sản phẩm
        // Hiển thị danh sách các QR đã tạo cho product này
        $(document).on('click', '.btn-view-product-qrs', (e) => {
            const productId = $(e.target).data('product-id');
            this.viewProductQRs(productId);
        });
        
        // SỰ KIỆN: Nút bắt đầu scan QR bằng camera
        // Mở camera và bắt đầu nhận diện QR
        $(document).on('click', '.btn-scan-qr', () => {
            this.startQRScanner();
        });
        
        // SỰ KIỆN: Nút dừng scan QR
        // Tắt camera và đóng scanner
        $(document).on('click', '.btn-stop-scan', () => {
            this.stopQRScanner();
        });
        
        // SỰ KIỆN: Input nhập QR thủ công
        // Cho phép người dùng nhập mã QR bằng bàn phím (không scan)
    ...
    /**
     * THIẾT LẬP CÁC MODAL POPUPS
     * 
     * Kiểm tra và tạo các modal nếu chưa tồn tại:
     * 1. qrScannerModal - Modal quét QR (camera + upload ảnh + nhập thủ công)
     * 2. qrListModal - Modal hiển thị danh sách QR codes
     * 3. qrDisplayModal - Modal hiển thị QR code vừa tạo
     * 
     * Sử dụng jQuery.length để kiểm tra element có tồn tại không
     */
    setupModalHandlers() {
        // Tạo modal scan QR nếu chưa có
        if (!$('#qrScannerModal').length) {
            this.createQRScannerModal();
        }
        
        // Tạo modal danh sách QR nếu chưa có
        if (!$('#qrListModal').length) {
            this.createQRListModal();
        }
        
        // Tạo modal hiển thị QR nếu chưa có
    ...
    /**
     * TẠO MÃ QR CODE MỚI CHO SẢN PHẨM
     * 
     * @param {number} productId - ID sản phẩm (bắt buộc)
     * @param {number|null} variantId - ID biến thể sản phẩm (tùy chọn)
     * 
     * Quy trình:
     * 1. Hiển thị loading spinner
     * 2. Gọi API với action='generate'
     * 3. Nếu thành công:
     *    - Hiển thị thông báo thành công
     *    - Mở modal hiển thị QR vừa tạo
     *    - Trigger event 'qr-generated' để các component khác biết
    ...
    async generateQR(productId, variantId = null) {
        try {
            ...
            // Bắt lỗi ngoại lệ (network error, timeout...)
            this.hideLoading();
    ...
    /**
     * IN LẠI MÃ QR CODE ĐÃ CÓ
     * 
     * @param {string} qrCode - Mã QR cần in lại (UUID hoặc mã QR)
     * 
     * Quy trình:
     * 1. Hiển thị loading "Đang chuẩn bị in..."
     * 2. Gọi API với action='reprint' để lấy URL trang in
     * 3. Mở popup window với URL trang in
     * 4. Kiểm tra popup có bị chặn không
     * 5. Ẩn loading và hiển thị kết quả
     * 
     * Lưu ý: 
     * - Cần tắt popup blocker để cửa sổ in mở được
     * - Popup có kích thước 800x600px
     */
    async reprintQR(qrCode) {
        try {
            ...
            // Bước 2: Gọi API lấy URL trang in
            const response = await this.apiCall({
                action: 'reprint',
                qr_code: qrCode
            });
            ...
            if (response.success) {
                ...
                // Mở cửa sổ popup mới với URL trang in
                // _blank: tab mới, width/height: kích thước popup
                const printWindow = window.open(response.print_url, '_blank', 'width=800,height=600');
                
                if (printWindow) {
                    // Popup mở thành công -> focus vào đó
                    printWindow.focus();
                } else {
                    // Popup bị chặn -> thông báo người dùng
    ...
    /**
     * XEM DANH SÁCH TẤT CẢ QR CODES CỦA SẢN PHẨM
     * 
     * @param {number} productId - ID sản phẩm cần xem QR
     * 
     * Quy trình:
     * 1. Hiển thị loading
     * 2. Gọi API với action='get_product_qrs'
     * 3. Nhận về mảng QR codes
     * 4. Hiển thị danh sách trong modal popup
     * 
     * Danh sách QR bao gồm:
     * - Mã QR (UUID)
     * - Trạng thái (active/inactive)
     * - Ngày tạo
     * - Các nút: In lại, Vô hiệu hóa
     */
    async viewProductQRs(productId) {
        try {
            ...
            // Bước 2: Gọi API lấy danh sách QR
            const response = await this.apiCall({
            ...
            // Bước 4: Xử lý kết quả
            if (response.success) {
                // Hiển thị danh sách QR trong modal
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <p><strong>Giá bán:</strong> ${product.price ? this.formatCurrency(product.price) : (product.unit_price ? this.formatCurrency(product.unit_price) : 'N/A')}</p>
                                <p><strong>Giá nhập:</strong> ${product.unit_price ? this.formatCurrency(product.unit_price) : 'N/A'}</p>
                                <p><strong>Vị trí lưu trữ:</strong> ${product.location_code || (product.qr_data && product.qr_data.location_code) || 'N/A'}</p>
                                <p><strong>Ngày nhập gần nhất:</strong> ${this.formatDate(product.last_receipt_date || product.receipt_date || product.created_at)}</p>
                            </div>
                        </div>
