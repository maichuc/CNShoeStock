...
try {
    ...
    header('Location: danh_sach_sp.php?success=product_deactivated&product_name=' . urlencode($product['name']));
    exit();
    
} catch (Exception $e) {
    header('Location: danh_sach_sp.php?error=deactivate_failed&message=' . urlencode($e->getMessage()));
    exit();
}
?>
