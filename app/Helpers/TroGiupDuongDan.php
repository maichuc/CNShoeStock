...
class PathHelper {
    ...
    public static function getImageUrl($filePath) {
        ...
        if (preg_match('/CNShoeStock\/(.+)$/', $filePath, $matches)) {
            return $matches[1];
        }
        
        // Pattern 2: Trích xuất everything after public/ folder
        if (preg_match('/public\/(.+)$/', $filePath, $matches)) {
            return $matches[1];
        }
        
        // Pattern 3: Trích xuất uploads/* or img/* pattern
