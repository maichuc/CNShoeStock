...
require_once __DIR__ . '/../../app/Classes/QuanLyNhanVien.php';
require_once __DIR__ . '/../../app/Classes/DichVuEmail.php';
...
try {
    switch ($action) {
        ...
        case 'check-username':
            ...
            if (empty($username)) {
                ob_clean(); echo json_encode(['success' => false, 'available' => false, 'message' => 'Username không được để trống']);
                exit;
            }
        ...
        case 'create':
            ...
            if (!in_array($userRole, ['admin', 'manager'])) {
                ...
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn không có quyền thực hiện thao tác này']);
                exit;
            }
            ...
            if ($userRole !== 'admin' && $requestData['warehouse_id'] != $warehouseId) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn chỉ có thể tạo nhân viên trong kho của mình']);
                exit;
            }
        ...
        case 'bulk-import':
            ...
            if (!isset($_FILES['file'])) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Không có file được upload']);
                exit;
            }
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadFile)) {
                ...
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Lỗi khi upload file: ' . $_FILES['file']['error']]);
            }
            break;
        ...
        case 'detail':
            ...
            if (empty($employeeId)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Thiếu user_id']);
                exit;
            }
            ...
            if (!$employee) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Nhân viên không tồn tại']);
                exit;
            }
            ...
            if ($userRole !== 'admin' && $employee['warehouse_id'] != $warehouseId) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn không có quyền xem nhân viên từ kho khác']);
                exit;
            }
        ...
        case 'update':
            if (!in_array($userRole, ['admin', 'manager'])) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn không có quyền cập nhật thông tin nhân viên']);
                exit;
            }
            
            $requestData = $data ?? [];
            $employeeId = $requestData['user_id'] ?? 0;
        ...
        case 'deactivate':
            if (!in_array($userRole, ['admin', 'manager'])) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn không có quyền vô hiệu hóa tài khoản']);
                exit;
            }
        ...
        case 'activate':
            if (!in_array($userRole, ['admin', 'manager'])) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn không có quyền kích hoạt tài khoản']);
                exit;
            }
        ...
        case 'activity-logs':
            ...
            if ($employeeId != $userId && !in_array($userRole, ['admin', 'manager', 'supervisor'])) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn không có quyền xem lịch sử của người khác']);
                exit;
            }
        ...
        case 'change-password':
            ...
            if (empty($currentPassword) || empty($newPassword) || empty($confirmPassword)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Vui lòng điền đầy đủ thông tin']);
                exit;
            }
            
            if ($newPassword !== $confirmPassword) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu xác nhận không khớp']);
                exit;
            }
            
            // Kiểm tra password policy
            if (strlen($newPassword) < 8) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu phải có ít nhất 8 ký tự']);
                exit;
            }
            
            if (!preg_match('/[A-Z]/', $newPassword) || !preg_match('/[a-z]/', $newPassword) || !preg_match('/[0-9]/', $newPassword)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu phải bao gồm chữ hoa, chữ thường và số']);
                exit;
            }
            ...
            if (!password_verify($currentPassword, $user['password_hash'])) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu hiện tại không chính xác']);
                exit;
            }
            
            // Kiểm tra new password != old password
            if (password_verify($newPassword, $user['password_hash'])) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu mới phải khác mật khẩu cũ']);
                exit;
            }
            ...
            ob_clean(); echo json_encode(['success' => true, 'message' => 'Đổi mật khẩu thành công']);
            break;
        ...
        case 'force-change-password':
            ...
            if (!preg_match('/[A-Z]/', $newPassword)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu phải có ít nhất 1 chữ hoa']);
                exit;
            }
            
            if (!preg_match('/[a-z]/', $newPassword)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu phải có ít nhất 1 chữ thường']);
                exit;
            }
            
            if (!preg_match('/[0-9]/', $newPassword)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu phải có ít nhất 1 chữ số']);
                exit;
            }
            
            if (!preg_match('/[@$!%*?&#]/', $newPassword)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu phải có ít nhất 1 ký tự đặc biệt (@$!%*?&#)']);
                exit;
            }
            ...
            if (!$user) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Người dùng không tồn tại']);
                exit;
            }
            ...
            // Kiểm tra new password != old password
            if (password_verify($newPassword, $user['password_hash'])) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Mật khẩu mới phải khác mật khẩu hiện tại']);
                exit;
            }
            ...
            if ($success) {
                ...
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Có lỗi xảy ra khi cập nhật mật khẩu']);
            }
            break;
        ...
        case 'admin-reset-password':
            ...
            if (empty($targetUserId)) {
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Thiếu thông tin user_id']);
                exit;
            }
            ...
            if ($userRole !== 'admin' && $targetUser['warehouse_id'] != $warehouseId) {
                http_response_code(403);
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Bạn chỉ có thể reset mật khẩu nhân viên trong kho của mình']);
                exit;
            }
            ...
            if ($success) {
                ...
                ob_clean(); echo json_encode(['success' => false, 'message' => 'Có lỗi xảy ra khi reset mật khẩu']);
            }
            break;
        ...
        case 'resend-welcome-email':
            ...
            if ($employee['warehouse_id']) {
                $stmt = $pdo->prepare("SELECT name FROM warehouses WHERE warehouse_id = :id");
        ...
        case 'generate-template':
            // Chuyển hướng hoặc include file tạo template
            $_GET['generate'] = 'template';
            if (ob_get_level()) ob_end_clean();
            require_once __DIR__ . '/../../app/Helpers/tao_mau_nhan_vien.php';
            exit;
