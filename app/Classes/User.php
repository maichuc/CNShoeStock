...
// Lớp Nhân viên kế thừa từ lớp User cơ sở
class Employee extends User {
    ...
    // Đa hình (Polymorphism): Hàm kiểm tra quyền cơ bản
    public function hasRole($roleToCheck) {
        return parent::hasRole($roleToCheck);
    }
}

// Lớp Quản lý kho kế thừa toàn bộ chức năng của Nhân viên
class WarehouseManager extends Employee {
    public function __construct($db) {
        parent::__construct($db);
        $this->role = 'manager';
    }

    // Đa hình (Polymorphism): Ghi đè hàm kiểm tra quyền
    public function hasRole($roleToCheck) {
        // Quản lý kho tự động kế thừa và có quyền truy cập vào các trang của nhân viên
        if (is_array($roleToCheck)) {
            return in_array('manager', $roleToCheck) || in_array('staff', $roleToCheck);
        }
        return $roleToCheck === 'manager' || $roleToCheck === 'staff';
    }
}

// Lớp Admin (nếu cần thiết)
...
// Factory Pattern để khởi tạo đúng loại đối tượng User dựa trên role
class UserFactory {
    public static function create($db, $role = null) {
        switch($role) {
            case 'admin':
                return new Admin($db);
            case 'manager':
                return new WarehouseManager($db);
            case 'staff':
