...
class SmartProductMatcher {
    ...
    /**
     * Tạo key để so sánh sản phẩm (nhóm các sản phẩm GIỐNG HỆT NHAU trong batch)
     */
    private function generateProductKey($result) {
        // Chuẩn hóa các giá trị
        $brand = strtolower(trim($result['brand'] ?? ''));
        // Lấy 2 từ đầu tiên của model để nhóm dễ hơn (tránh khác biệt nhỏ ở cuối)
        $model = strtolower(trim($result['model'] ?? ''));
        $modelWords = explode(' ', $model);
        $modelBase = implode(' ', array_slice($modelWords, 0, 2));
        ...
        // Tạo key unique (Brand + Model Base + Color + Size)
        return "{$brand}|{$modelBase}|{$color}|{$size}";
    }
    ...
    public function findSimilarInDatabase($product) {
        try {
            // Tìm kiếm by brand and name
            $sql = "SELECT p.product_id, p.name, p.brand, p.description,
                           pv.variant_id, pv.sku, pv.color, pv.size, pv.price
                    FROM products p
                    LEFT JOIN product_variants pv ON p.product_id = pv.product_id
                    WHERE (LOWER(p.name) LIKE ? OR LOWER(p.brand) LIKE ? OR LOWER(p.description) LIKE ?)
                    LIMIT 20";
            
            $brandSearch = '%' . strtolower($product['brand'] ?? '') . '%';
            $nameSearch = '%' . strtolower($product['model'] ?? '') . '%';
            $descSearch = '%' . strtolower($product['color'] ?? '') . '%';
            
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([$nameSearch, $brandSearch, $descSearch]);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            
            $matches = [];
            foreach ($results as $dbProduct) {
                // Sử dụng hàm similarity từ helper (TroGiupDoTuongDong.php)
                $similarity = calculateEnhancedSimilarity([
                    'brand' => $product['brand'] ?? '',
                    'name' => $product['model'] ?? '', // AI model -> DB name
                ...
                if ($similarity >= 60) { // Giảm threshold xuống 60% để bắt được nhiều trùng lặp hơn
                    $matches[] = [
