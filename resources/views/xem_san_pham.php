...
require_once __DIR__ . '/../../app/Helpers/TroGiupDuongDan.php';
...
// 🔓 Lấy product_id từ encrypted ID trong URL
$product_id = null;

if (isset($_GET['id'])) {
    // Giải mã ID
    $product_id = SecurityHelper::decryptId($_GET['id']);
...
$sql_variants = "
    ...
    LEFT JOIN product_qr_codes qr ON qr.variant_id = pv.variant_id AND qr.is_active = 1
...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <div>
                            <?php
                            // 🔐 Mã hóa ID để chỉnh sửa
                            $encrypted_edit_id = SecurityHelper::encryptId($product_id);
                            ?>
                            <a href="sua_san_pham.php?id=<?php echo urlencode($encrypted_edit_id); ?>" class="btn btn-warning btn-sm mr-2">
                                    ...
                                    <div class="variant-card">
                                        <div class="row">
                                            ...
                                            <div class="col-md-3 text-center">
                                                <?php if (!empty($variant['qr_image_path'])): ?>
                                                    <div class="info-label">Mã QR:</div>
                                                    <img src="<?php echo htmlspecialchars($variant['qr_image_path']); ?>" 
                                                         alt="QR Code" 
                                                         class="qr-code-img mb-2"
                                                         style="max-width: 100px; max-height: 100px; cursor: pointer;"
                                                         onclick="downloadQRCode('<?php echo htmlspecialchars($variant['qr_image_path']); ?>', '<?php echo htmlspecialchars($variant['sku']); ?>')">
                                                    <br>
                                                    <button class="btn btn-sm btn-primary" onclick="downloadQRCode('<?php echo htmlspecialchars($variant['qr_image_path']); ?>', '<?php echo htmlspecialchars($variant['sku']); ?>')">
    ...
    <script>
    function downloadQRCode(qrPath, sku) {
        // Tạo element ảnh tạm để load QR code
        const img = new Image();
        img.crossOrigin = 'anonymous';
        
        img.onload = function() {
            // Tạo canvas để vẽ QR code
        ...
        img.onerror = function() {
            // Nếu không load được ảnh, thử download trực tiếp
            const a = document.createElement('a');
            a.href = qrPath;
            a.download = 'QR_' + sku + '.png';
            a.target = '_blank';
    ...
    function showNotification(message, type) {
        ...
        const notification = $('<div class="alert ' + alertClass + ' alert-dismissible fade show position-fixed" role="alert" style="top: 80px; right: 20px; z-index: 9999; min-width: 300px;">' +
