...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            throw new Exception('Yêu cầu không hợp lệ (CSRF token mismatch)');
        }
        ...
        if ($stmt->execute()) {
            ...
            // 🔐 Mã hóa ID cho redirect
            $encrypted_redirect_id = SecurityHelper::encryptId($product_id);
            $_SESSION['encrypted_redirect_id'] = $encrypted_redirect_id;
...
$typeStmt = $pdo->prepare("
    ...
    FROM products
    WHERE type IS NOT NULL AND type != '' AND warehouse_id = :warehouse_id
    ORDER BY type ASC
");
$typeStmt->bindParam(':warehouse_id', $userWarehouseId, PDO::PARAM_INT);
$typeStmt->execute();
$product_types = [];
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <div>
                            <?php
                            // 🔐 Mã hóa ID để xem chi tiết
                            $encrypted_view_id = SecurityHelper::encryptId($product_id);
                            ?>
                            <a href="xem_san_pham.php?id=<?php echo urlencode($encrypted_view_id); ?>" class="btn btn-info btn-sm mr-2">
    ...
    <script>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = 'xem_san_pham.php?id=<?php echo urlencode($_SESSION['encrypted_redirect_id'] ?? ''); ?>';
            }
        });
        <?php endif; ?>
